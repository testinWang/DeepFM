...
tf.app.flags.DEFINE_integer("feature_size", 18164, "Number of features") #每个feature, one-hot之后的的维度
tf.app.flags.DEFINE_integer("field_size", 39, "Number of fields")#field_size表示原始特征的数量
tf.app.flags.DEFINE_integer("embedding_size", 32, "Embedding size")# 隐含变量K值的选取
tf.app.flags.DEFINE_integer("num_epochs", 1, "Number of epochs")  #所有数据训练遍历epoch次
tf.app.flags.DEFINE_integer("batch_size", 128, "Number of batch size")
tf.app.flags.DEFINE_integer("log_steps", 2000, "save summary every steps")
tf.app.flags.DEFINE_float("learning_rate", 0.0005, "learning rate")
tf.app.flags.DEFINE_float("l2_reg", 0.0001, "L2 regularization")
tf.app.flags.DEFINE_string("loss_type", 'log_loss', "loss type {square_loss, log_loss}")
tf.app.flags.DEFINE_string("optimizer", 'Adam', "optimizer type {Adam, Adagrad, GD, Momentum}")
tf.app.flags.DEFINE_string("deep_layers", '256,128,64', "deep layers")
tf.app.flags.DEFINE_string("dropout", '0.7,0.7,0.7', "dropout rate")
tf.app.flags.DEFINE_boolean("batch_norm", False, "perform batch normaization (True or False)")
tf.app.flags.DEFINE_float("batch_norm_decay", 0.9, "decay for the moving average(recommend trying decay=0.9)")
tf.app.flags.DEFINE_string("data_dir", 'F:\CTR\FM\criteo_data', "data dir")
tf.app.flags.DEFINE_string("dt_dir", '', "data dt partition")
tf.app.flags.DEFINE_string("model_dir", 'F:\CTR\FM\model_dir', "model check point dir")
tf.app.flags.DEFINE_string("servable_model_dir", 'F:\CTR\FM\servable_model_dir', "export servable model for TensorFlow Serving")
tf.app.flags.DEFINE_string("task_type", 'export', "task type {train, infer, eval, export}")
   ...
   #------build feaure-------
    ...
    with tf.variable_scope("Deep-part"):
        if FLAGS.batch_norm:
            ...
            if mode == tf.estimator.ModeKeys.TRAIN: #tf.estimator.ModeKeys.TRAIN='train'
                train_phase = True
...
def main(_):
    ...
    if FLAGS.clear_existing_model and FLAGS.task_type=='train':  #清理模型
        try:
            shutil.rmtree(FLAGS.model_dir)   #递归地清空该文件夹
    ...
    #RunConfig()指定Estimator运行的配置， replace()替换新实例中指定的属性
    ...
    if FLAGS.task_type == 'train':
        train_spec = tf.estimator.TrainSpec(input_fn=lambda: input_fn(tr_files, num_epochs=FLAGS.num_epochs, batch_size=FLAGS.batch_size), max_steps=10000) #可以在此处设置组最大的迭代步数
    ...
    #导出模式
        ...
        #tensorflow serving 其中serving_input_receiver_fn接受推理请求
